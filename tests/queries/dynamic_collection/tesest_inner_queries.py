...
collection_name = "dynamic_inner_ticket"


class InnerTicket(DynamicCollectionDocument):
    name: str
    position: int
    config: dict
    params: dict
...
async def innert_tickets(event_loop):
    ...
    await InnerTicket.Q(collection_name).insert_one(
        name="first",
        position=1,
    ...
    yield
    await InnerTicket.Q(collection_name).drop_collection(force=True)


@pytest.mark.asyncio
async def test_update_many(connection):
    # .create_documents()
    updated = await InnerTicket.Q(collection_name).update_many(
        position__range=[3, 4], name__ne="hhh", config__url__set="test.io"
    )
    assert updated == 2
    last = await InnerTicket.Q(collection_name).find_one(position=4)
...
async def test_inner_find_one_in_array(connection):
    data = await InnerTicket.Q(collection_name).find_one(array__1__regex="2")
    assert data is not None
    assert data.name == "second"

    data = await InnerTicket.Q(collection_name).find_one(
        array__1__regex="2", array__0__regex="1"
    )
    assert data is None

    data = await InnerTicket.Q(collection_name).find_one(array__0="1")
    assert data is not None
...
async def test_inner_update_one(connection):
    updated = await InnerTicket.Q(collection_name).update_one(
        config__url__startswith="goo", config__url__set="test.io"
    )
    assert updated == 1
    data = await InnerTicket.Q(collection_name).find_one(config__url__startswith="test")
    assert data is not None
    assert data.name == "second"
