...
product_collection = "dynamic_product"
product_image_collection = "dynamic_image"
place_collection = "dynamic_place"
...
async def innert_producs(event_loop):
    ...
    for product_id in products:
        for i in range(1, 5):
            ...
            # print(img)
            _ = await img.save(product_image_collection)
    yield
    await Product.Q(product_collection).drop_collection(force=True)
    await ProductImage.Q(product_image_collection).drop_collection(force=True)
...
async def test_aggregate_method(connection):
    aggregate = (
        Product.manager.aggregate(product_collection)
    ...
    assert result.native_result == [{"_id": {"quant": 2}}]
    with pytest.raises(ValueError):
        image_aggreagate = (
            ProductImage.manager.aggregate(product_image_collection)
            .lookup(
                Product,
...
async def test_geo(connection):
    ...
    class Place(DynamicCollectionDocument):
        name: str
        location: dict
        category: str
    ...
    await Place.ensure_indexes(place_collection)
    ...
    await Place.Q(place_collection).insert_many(data)

    geo_aggregate = (
        Place.manager.aggregate(product_collection)
        .geo_near(
    ...
    await Place.Q(place_collection).drop_collection(force=True)


@pytest.mark.asyncio
async def test_raises_invalid_field(connection):
    with pytest.raises(MotordanticValidationError):
        await Product.Q(product_collection).simple_aggregate(
...
async def test_raw_aggregate(connection):
    result_raw_group_by_by_inners = await Product.Q(product_collection).raw_aggregate(
        data=[
            {
                "$group": {
