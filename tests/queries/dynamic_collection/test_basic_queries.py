...
class Ticket(DynamicCollectionDocument):
    name: str
    position: int
    config: dict
    sign: int = 1
...
class Trash(DynamicCollectionDocument):
    name: str
    date: str
...
async def drop_ticket_collection(event_loop):
    yield
    await Ticket.Q("dynamic_ticket").drop_collection(force=True)
    await Trash.Q("dynamic_trash").drop_collection(force=True)
...
async def test_insert_one(connection):
    ...
    object_id = await Ticket.Q("dynamic_ticket").insert_one(**data)
...
async def test_insert_many(connection):
    ...
    inserted = await Ticket.Q("dynamic_ticket").insert_many(data)
    assert inserted == 3


@pytest.mark.asyncio
async def test_distinct(connection):
    data = await Ticket.Q("dynamic_ticket").distinct("position", name="second")
    assert data == [2]


@pytest.mark.asyncio
async def test_queryset_serialize(connection):
    result = await Ticket.Q("dynamic_ticket").find(name="second")
...
async def test_find_one(connection):
    ...
    data = await Ticket.Q("dynamic_ticket").find_one(name="second")
    second = await Ticket.Q("dynamic_ticket").find_one(_id=data._id)
...
async def test_find(connection):
    result = await Ticket.Q("dynamic_ticket").find(limit_rows=1, name="second")
...
async def test_async_get(connection):
    ...
    data = await Ticket.Q("dynamic_ticket").get(name="second")
    second = await Ticket.Q("dynamic_ticket").get(_id=data._id)
    assert isinstance(data, DynamicCollectionDocument)
    assert data.name == "second"  # type: ignore
    assert data.position == 2  # type: ignore
    assert second._id == data._id
    with pytest.raises(DoesNotExist):
        _ = await Ticket.Q("dynamic_ticket").get(name="invalid_name")


@pytest.mark.asyncio
async def test_save(connection):
    obj = await Ticket.Q("dynamic_ticket").find_one(name="second")
    obj.position = 2310
    obj.name = "updated"
    await obj.save("dynamic_ticket")
    none_obj = await Ticket.Q("dynamic_ticket").find_one(name="second", position=222222)
    assert none_obj is None
    new_obj = await Ticket.Q("dynamic_ticket").find_one(_id=obj._id)
    ...
    await obj.save("dynamic_ticket")
    last_obj = await Ticket.Q("dynamic_ticket").find_one(name="second")
...
async def test_update_one(connection):
    updated = await Ticket.Q("dynamic_ticket").update_one(
...
async def test_update_many(connection):
    data = await Ticket.Q("dynamic_ticket").update_many(
        name="third", config__set={"updated": 3}
    )
    updated = await Ticket.Q("dynamic_ticket").find_one(name="third")
    assert data == 2
...
async def test_gte_lte_in_one_field(connection):
    ...
    await Trash.Q("dynamic_trash").insert_one(name="first", date="2022-01-01")
    await Trash.Q("dynamic_trash").insert_one(name="second", date="2022-01-01")
    await Trash.Q("dynamic_trash").insert_one(name="third", date="2022-01-03")
    await Trash.Q("dynamic_trash").insert_one(name="4", date="2022-01-05")

    data = await Trash.Q("dynamic_trash").count(
...
async def test_find_with_regex(connection):
    second = await Ticket.Q("dynamic_ticket").find_one(name__regex="ond")
    assert second.name == "second"

    starts = await Ticket.Q("dynamic_ticket").find_one(name__startswith="s")
    assert starts._id == second._id

    istarts = await Ticket.Q("dynamic_ticket").find_one(name__istartswith="S")
    assert istarts._id == second._id

    not_istarts = await Ticket.Q("dynamic_ticket").find_one(
...
async def test_find_and_update(connection):
    data_default = await Ticket.Q("dynamic_ticket").find_one_and_update(
        name="second", position__set=23
    )
    assert data_default.position == 23

    data_with_prejection = await Ticket.Q("dynamic_ticket").find_one_and_update(
...
async def test_session_find(connection):
    session = await Ticket.manager._start_session()
    ticket = await Ticket.Q("dynamic_ticket").find_one(session=session)
...
async def test_session_context(connection):
    async with Session(Ticket.manager) as session:
        ticket = await Ticket.Q("dynamic_ticket").find_one(session=session)
        assert ticket is not None
        new_ticket_id = await Ticket.Q("dynamic_ticket").insert_one(
            name="session ticket",
            position=100,
        ...
        deleted = await Ticket.Q("dynamic_ticket").delete_one(_id=new_ticket_id)
        assert deleted == 1


@pytest.mark.asyncio
async def test_delete_one(connection):
    deleted = await Ticket.Q("dynamic_ticket").delete_one(name="second")
    assert deleted == 1


@pytest.mark.asyncio
async def test_delete_many(connection):
    deleted = await Ticket.Q("dynamic_ticket").delete_many(name="second")
    assert deleted == 1
